...
function Del(id){
	if(confirm("确定该操作吗？")){
	    location.href = "bannerDel?baid="+id;
	}
}
function add(){
		...
		<table class="default" width="100%">
			...
			<c:choose>
				<c:when test="${not empty list}">
					<c:forEach items="${list}" var="ba" varStatus="vs">
						<tr class="list">
							<td class='center' style="width: 30px;">${vs.index+1}</td>
							<td><img alt="" src="<%=basePath%>${ba.banner_image }" class="img"> </td>
							<td>
							    <c:choose>
							        <c:when test="${ba.banner_type ==0 }">非轮播图</c:when>
							        <c:when test="${ba.banner_type ==1 }">轮播图</c:when>
							        <c:when test="${ba.banner_type ==2 }">欢迎页</c:when>
							        <c:when test="${ba.banner_type ==3 }">引导页</c:when>
							    </c:choose>
							</td>
							<td>${ba.banner_order }</td>
							<td> 
							    <c:forEach items="${listmodel }" var="model">
							        <c:if test="${model.mid == ba.model_id }">模板${model.m_order }</c:if>
							    </c:forEach>
							   
							</td>
							<td>${ba.banner_createdate }</td>
							<td>
							    <a href="javascript:void" onclick="Del('${ba.baid }')">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
