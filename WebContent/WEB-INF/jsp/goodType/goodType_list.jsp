...
function Del(id){
	if(confirm("确定该操作吗？")){
	    location.href = "goodTypeDel.do?id="+id;
	}
}
function add(){
...
<body>
	...
	<form action="goodType/goodTypeList.do" method="post"id="form0"></form>
    <form action="goodTypeList.do" method="post" id="form1">
	<div id="tips">
		<div id="buttonGroup">
		...
		<table class="default" width="100%">
			...
			<c:choose>
				<c:when test="${not empty list}">
					<c:forEach items="${list}" var="goodtype" varStatus="vs">
						<tr class="list">
							<td class='center' style="width: 30px;">${vs.index+1}</td>
							<td>${goodtype.ft_foodtype_name }</td>
							<td>${goodtype.ft_foodtype_remark }</td>
							<td>${goodtype.ft_foodtype_createdate }</td>
							<td>
							    <a href="javascript:void" onclick="Del('${goodtype.ftid }')">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
