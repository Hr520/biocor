...
<script type="text/javascript" charset="UTF-8">
	...
	function Del(umid) {
		if(confirm("确定该操作吗？")){
		    location.href = "deleteMessage.do?umid=" + umid;
		}
	}
	function add() {
		location.href = "jupAddmsg.do";
	}
	function sendTUI(um_msgcontent, umid) {
		console.log("TUImessage.do?um_msgcontent="+ encodeURIComponent(um_msgcontent) + "&" + "umid="+ umid);
		
		location.href = "TUImessage.do?um_msgcontent="
				+ encodeURIComponent(um_msgcontent) + "&" + "umid="
				+ umid;
	...
	<div id="navi">
		<div id='naviDiv'>
			<span><img src="<%=basePath%>/res/images/arror.gif" width="7"
				...
				border="0" alt=""> </span> &nbsp;推送列表&nbsp;
		</div>
	</div>
	<form action="messege/geteMessageList.do" method="post" id="form0"></form>
	<form action="geteMessageList.do" method="post" id="form1">
		<div id="tips">
			<div id="buttonGroup">
					...
					<c:when test="${not empty list}">
						<c:forEach items="${list}" var="message" varStatus="vs">
							<tr class="list">
								...
								<td>${message.um_msgtitle }</td>
								<td>${message.um_msgcontent }</td>
								<td>${message.um_msgdate}</td>
								<td><c:if test="${message.u_tuistate==1}">
										<a href="javascript:void"
											onclick="sendTUI('${message.um_msgcontent}',${message.umid})">推送</a>
									</c:if> <c:if test="${message.u_tuistate!=1}">
									已推送
									</c:if>
									<c:if test="${sessionUser.ROLE_ID=='1'}">
								<td>
								<a href="javascript:void"
									onclick="Del('${message.umid }')">删除</a>
									</td>
									</c:if>
							</tr>
						</c:forEach>
