...
<script type="text/javascript">
	...
	function del(qid){
		if(confirm("确定该操作吗？")){
			location.href="questDel.do?qid="+qid;
		}
	}
</script>
<body>
		...
		<table class="default" width="100%">
			...
			<c:when test="${not empty list}">
				<c:forEach items="${list}" var="list" varStatus="vs">
					<tr>
					    <td>${fn:substring(list.q_qNO,0,50) }</td>
						<td>${list.q_qname }</td>
						<td>
						    <c:choose>
						        <c:when test="${list.q_type==0 }">单选</c:when>
						        <c:when test="${list.q_type==1 }">多选</c:when>
						    </c:choose>
						</td>
						<td>${list.q_createdate }</td>
						<td>${fn:substring(list.q_remark,0,15) }</td>
						<td>
							<a href="questAnswerList.do?qid=${list.qid }">答案选项</a>
							&nbsp;|&nbsp;
							<a href="javascript:void;" onclick="del('${list.qid}')">删除</a>
						</td>
					</tr>
				</c:forEach>
			</c:when>
