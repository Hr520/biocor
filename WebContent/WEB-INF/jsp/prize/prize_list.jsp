...
<script type="text/javascript">
	...
	function edit(pgid){
		location.href = "prizeEdit.do?pgid="+pgid;
	}
</script>
<body>
	...
	<div id="mainContainer">
		<table class="default" width="100%">
			...
			<c:when test="${not empty list}">
				<c:forEach items="${list}" var="prize" varStatus="vs">
					<tr>
					    <td>${vs.index + 1 }</td>
						<td>${prize.prizeg_gift_name }</td>
						<td>${prize.level }</td>
						<td>${prize.prizeg_gift_probability }</td>
						<td>${prize.prizeg_createdate }</td>
					    <td>${fn:substring(prize.prizeg_remark,0,10) }</td>
						<td><a href="javascript:void" onclick="edit('${prize.pgid }')">编辑</a>
						</td>
					</tr>
				</c:forEach>
			</c:when>
