...
function exportexcel(){
	location.href="shopstatisticalExcel.do?"
}
</script>
<body>
	<div id="navi">
		<div id='naviDiv'>
			<span><img src="<%=basePath%>/res/images/arror.gif" width="7"
				height="11" border="0" alt=""> </span> &nbsp;财务统计&nbsp; <span>
				<img src="<%=basePath%>/res/images/arror.gif" width="7" height="11"
				border="0" alt=""> </span> &nbsp;店铺收益详情列表&nbsp;
		</div>
	</div>
	...
	<form action="financial/Getfinancial.do" method="post" id="form0"></form>
					...
					<c:when test="${not empty list}">
						<c:forEach items="${list}" var="list" varStatus="vs">
							<tr class="list">
								...
								<td>${list.order_total}笔</td>
								<td>${list.order_total_day}笔</td>
								<td>${list.order_money}元</td>
								<td>${list.order_money_day}元</td>
								<td>${list.order_total_spending}笔</td>
								<td>${list.order_total_Notspending}笔</td>
								<td>${list.order_total_appointment}笔</td>
								<td>${list.order_total_Cancel}笔</td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
