...
<script type="text/javascript">
	...
	function Commend(sid,state){
		var num = "${num }";
		if(num<10){
			location.href="recommendState.do?id="+sid+"&state="+state+"&type=1";
		}else{
			alert("已经达到推荐上限了!");
		}
			...
			<c:when test="${not empty list}">
				<c:forEach items="${list}" var="shop" varStatus="vs">
					<tr>
						...
						<td>
							<c:choose>
							    <c:when test="${shop.shop_ISrecommend ==1 }">推荐</c:when>
							    <c:when test="${shop.shop_ISrecommend ==0 }">不推荐</c:when>
							</c:choose>
					    </td>
						<td>
						    <c:choose>
						...
						<td>
					        <c:choose>
					     	    <c:when test="${shop.shop_ISrecommend ==1 }">
					     	        <a href="javascript:void" onclick="Commend('${shop.sid }','0')">不推荐</a>
					     	    </c:when>
					     	    <c:when test="${shop.shop_ISrecommend ==0 }">
					     	        <a href="javascript:void" onclick="Commend('${shop.sid }','1')">推荐</a>
					     	    </c:when>
					     	</c:choose>
						</td>
					</tr>
