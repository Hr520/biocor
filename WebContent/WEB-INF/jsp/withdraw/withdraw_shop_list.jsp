...
<script type="text/javascript">
	...
	function down(){
		var type = $("#type").val();
		location.href = "downShop.do?type="+type;
	}
	
	function changeState(id){
		location.href = "befChangeShop.do?id="+id;
	}
</script>
<body>
    <form action="withdraw/withdrawShopList.do" method="post" id="form0" style="">
    </form>
	<div id="navi">
		<div id='naviDiv'>
			<span>
	...
	<form action="withdrawShopList.do" method="post" id="form1">
	<div id="tips">
	    <div id="buttonGroup">
		    &nbsp;类型查询:
	...
	<div id="mainContainer">
		<table class="default" width="100%">
			...
			<c:when test="${not empty list}">
				<c:forEach items="${list}" var="wshop" varStatus="vs">
					<tr>
					    <td>${vs.index + 1 }</td>
						<td>${wshop.shop_name }</td>
						<td>${wshop.money }</td>
						<td>${wshop.getmoney_time }</td>
						<td>
							<c:choose>
							    <c:when test="${wshop.state ==0 }">申请中</c:when>
							    <c:when test="${wshop.state ==1 }">成功</c:when>
							    <c:when test="${wshop.state ==2 }">失败</c:when>
							</c:choose>
					    </td>
						<td>${wshop.agreen_time }</td>
					    <td>${fn:substring(wshop.remark,0,10) }</td>
						<td>
						    <c:choose>
						        <c:when test="${wshop.state == 1 || wshop.state == 2 }">
						            <span>完成</span>
						        </c:when>
							    <c:when test="${wshop.state == 0 }">
							        <a href="javascript:void" onclick="changeState('${wshop.id }')">编辑</a>
							    </c:when>
						    </c:choose>
						</td>
					</tr>
