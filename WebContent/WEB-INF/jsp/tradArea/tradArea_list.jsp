...
function Del(id){
	if(confirm("确定该操作吗？")){
	    location.href = "tradAreaDel.do?id="+id;
	}
}
function add(){
...
<body>
	...
	<form action="tradArea/tradAreaList.do" method="post"id="form0"></form>
    <form action="tradAreaList.do" method="post" id="form1">
	<div id="tips">
		<div id="buttonGroup">
		...
		<table class="default" width="100%">
			...
			<c:choose>
				<c:when test="${not empty list}">
					<c:forEach items="${list}" var="trad" varStatus="vs">
						<tr class="list">
							...
							<td>${trad.street }</td>
							<td>${trad.streetID }</td>
							<td>${trad.hats_remark }</td>
							<td>${trad.hats_createdate }</td>
							<td>
							    <a href="javascript:void" onclick="Del('${trad.id }')">删除</a>
							</td>
						</tr>
					</c:forEach>
				</c:when>
